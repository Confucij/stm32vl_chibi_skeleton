...
void __attribute__  ((visibility ("protected"))) Dummy_Handler(void) {
    while(1) {
        ;
    }
}
 ...
 */
volatile void *vector_table[] __attribute__ ((section(".vectors"), visibility ("protected"))) = {
	&_estack,
	Reset_Handler,
	NMI_Handler,
	HardFault_Handler,
...
void __attribute__  ((visibility ("protected")))  SystemInit(void) {
    ;
}

void __attribute__((noreturn, naked, visibility ("protected"))) Reset_Handler() {
    unsigned long *src;
    unsigned long *dest;
    if(!vector_table[0])
        while(1){;}
